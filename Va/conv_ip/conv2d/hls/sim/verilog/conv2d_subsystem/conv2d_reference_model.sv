...
class conv2d_reference_model extends uvm_component;
    ...
    endgroup
    int svr_weight_stream_delay;
    covergroup svr_weight_stream_cov;
        delay: coverpoint svr_weight_stream_delay
        {
            bins norm[67] = { [0 : 66] };
        }
    endgroup
    ...
    //  trans_size++;
        svr_weight_stream_delay = tr.delay;
        svr_weight_stream_cov.sample();
        `uvm_info(this.get_full_name(), "port a collected one pkt", UVM_DEBUG);
    endfunction
